 ...
 */
export function deriveHDNode(hdKey: string, derivationPath: string, network: DashjsNetwork): HDNode {
...
export function deriveKeyPair(baseNode: HDNode, index: number, network: DashjsNetwork): KeyPair {
  return baseNode.derive(0).derive(index)
}

export function deriveAddress(
  baseNode: HDNode, index: number, network: DashjsNetwork, addressType: AddressType,
): string {
  const keyPair = deriveKeyPair(baseNode, index, network)
  return publicKeyToAddress(keyPair.publicKey, network, addressType)
}

export function derivePrivateKey(baseNode: HDNode, index: number, network: DashjsNetwork) {
  const keyPair = deriveKeyPair(baseNode, index, network)
  return keyPair.toWIF()
}

export function xprvToXpub(xprv: string, derivationPath: string, network: DashjsNetwork) {
