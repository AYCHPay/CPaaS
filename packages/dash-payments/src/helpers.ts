...
import { DashjsNetwork, AddressType } from './types'
...
export function isValidXprv(xprv: string, network: DashjsNetwork): boolean {
  try {
...
export function isValidXpub(xpub: string, network: DashjsNetwork): boolean {
  try {
...
export function validateHdKey(hdKey: string, network: DashjsNetwork): string | undefined {
  try {
...
export function isValidAddress(address: string, network: DashjsNetwork): boolean {
  try {
...
export function publicKeyToAddress(publicKey: Buffer, network: DashjsNetwork, addressType: AddressType): string {
...
function privateKeyToKeyPair(privateKey: string, network: DashjsNetwork) {
  return bitcoin.ECPair.fromWIF(privateKey, network)
}

export function privateKeyToAddress(privateKey: string, network: DashjsNetwork, addressType: AddressType) {
  const keyPair = privateKeyToKeyPair(privateKey, network)
  return publicKeyToAddress(keyPair.publicKey, network, addressType)
}

export function isValidPrivateKey(privateKey: string, network: DashjsNetwork): boolean {
  try {
